...
from keras.layers import Dropout, Dense, \
    concatenate, LSTM, Embedding, Concatenate
...
from src.train.loader.EmbeddingLoader import EmbeddingLoader, load_word2vec
...
class MSREnsembleNN(Trainer):
    ...
    def train_ensemble(self, ensemble_x, ensemble_y):
        ensemble_y = np_utils.to_categorical(ensemble_y)
        ...
        def idx2word(idx):
            return self._word2vec.wv.index2word[idx]

        print('\nPreparing the data for LSTM...')
        # One input for correcting the order of predicted emails
        # and one to fit the bug vector directly
        train_x_pred = np.zeros([len(ensemble_x), max_sequence_length], dtype=np.int32)
        train_x_bug = np.zeros([len(ensemble_x), max_sequence_length], dtype=np.int32)
        ...
        train_x_pred = train_x_pred.reshape(len(ensemble_x), max_sequence_length, 1)
        train_x_bug = train_x_bug.reshape(len(ensemble_x), max_sequence_length, 1)

        input1 = Input(shape=(max_sequence_length, 1))
        input2 = Input(shape=(max_sequence_length, 1))

        lstm1 = LSTM(emdedding_size)(input1)
        lstm2 = LSTM(emdedding_size)(input2)
        dropout1 = Dropout(0.20)(lstm1)
        dropout2 = Dropout(0.20)(lstm2)
        merged = concatenate([dropout1, dropout2])
        final_dropout = Dropout(0.4)(merged)
        output = Dense(units=ensemble_y.shape[1], activation='softmax')(final_dropout)

        self._ensemble = Model(input=(input1, input2), output=output)

        self._ensemble.compile(loss='categorical_crossentropy', optimizer='adam',
                               metrics=['accuracy'])

        self._ensemble.fit([train_x_pred, train_x_bug], ensemble_y,
    ...
    def _bug_to_vector(self, predictions, bug_vector):
        ...
        pred_ved = np.zeros((1, max_sequence_length), dtype=np.int32)
        bug_vec = np.zeros((1, max_sequence_length), dtype=np.int32)
        for t, word in enumerate(sentence):
            if t == max_sequence_length:
                break
            pred_ved[0, t] = self._word2idx(word)

        pred_ved = pred_ved.reshape(1, max_sequence_length, 1)
        ...
        bug_vec = bug_vec.reshape(1, max_sequence_length, 1)
